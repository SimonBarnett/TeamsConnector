...
- [ ] Path B hear: `Calls.AccessMedia.All` **granted** 2026-09-17. Azure STT + PCM ring shipped. Windows `services/media-host` (Calls.Media 1.2.0.17950, audio Sendrecv) joins with `applicationHostedMediaConfig`. **2026-09-18 IONOS `WIN-MPRE8VI4U6U` / `rtp-teams.ntsa.uk`:** Graph admit + `/callback` 202 work; media dies ~1s with **500#1203002** (never `established`). LE + ZeroSSL + TLS 1.2 + USERTrust chain + Azure issuing CAs did not clear it. Microsoft requires **Azure Windows IaaS** for app-hosted media — see **`docs/path-b-ionos-1203002.md`**. **Do not** flip `plane=auto`. Azure F1 stays Path A.
