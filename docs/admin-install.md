...
5. **Application access policy** — without this, Graph returns 403/404 and the connector reports `policy_missing` (not `meeting_not_found`). Doctor probes `GET /users/{GRAPH_USER_ID}/onlineMeetings` with a dummy JoinWebUrl filter (Graph rejects unfiltered `$top=1`).
